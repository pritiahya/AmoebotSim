...
{
  // First, handle movement. If the particle is contracted, choose a random
  ...
  if(expandable) 
  {
      for (int label : headLabels())
      {
        if (hasNbrAtLabel(label))
        {
          ...
          expandable = false;
          //neighbor.expandable = false;
          changableState = false;
          neighbor.changableState = false;
          if (isExpanded())
            contractTail();
          /*if (isHeadLabel(label) && neighbor.isExpanded())
          {
            neighbor.contractTail();
          }
          if (neighbor.isExpanded() && isTailLabel(label))
          {
            neighbor.contractHead();
          }*/
          break;
          }
        }
  ...
  // Next decrement the particle's counter. If it's zero, reset the counter and
  // get a new color.
  _counter--;
  if (_counter == 0)
  {
